...
public class PersonProntuaryService {
    ...
    public ResponseEntity<PersonPacientProntuaryOutDto> persist(Long personId, PersonPacientProntuaryInDto dto) {
        ...
        throw new NotFoundException(ExceptionMessage.PRONTUARY_NOT_FOUND);
    }
    ...
    public List<PersonPacientProntuaryOutDto> findAllById(Long personId) {
        ...
        if (!prontuaries.isEmpty()) {
            ...
            prontuaries.forEach(personPacientProntuary -> {
                PersonPacientProntuaryOutDto entityNew = modelMapper.map(personPacientProntuary, PersonPacientProntuaryOutDto.class);
                entityNew.setPersonName(personPacientProntuary.getPacient().getPersonName());
                prontuaryOutDtoList.add(entityNew);
            });
    ...
    /**Convert a ProntuaryInDto to entity.*/
